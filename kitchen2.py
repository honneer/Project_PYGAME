...
def kitchen_page(username, pet_name, stats):
    ...
    # Check files exist
    missing_files = [path for path in [bg_path, pet_path, fridge_open_path] + list(food_paths.values()) if not os.path.isfile(path)]
    if missing_files:
    ...
    pet_img = pygame.image.load(pet_path).convert_alpha()
    pet_img = pygame.transform.smoothscale(pet_img, (300, 300))
    ...
    while running:
        ...
        screen.blit(pet_img, (pet_rect.x, pet_rect.y + offset))

        font = pygame.font.SysFont(None, 36)
        hunger_text = font.render(f"Hunger: {stats['Kitchen']}", True, (0, 0, 0))
